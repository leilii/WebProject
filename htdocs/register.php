...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // Handle the form.
	...
	if (preg_match('/^[A-Z \'.-]{2,20}$/i', $trimmed['first_name'])) {
		...
		$fn=trim(htmlspecialchars($_POST['first_name']));
	} else {
	...
	if (preg_match('/^[A-Z \'.-]{2,40}$/i', $trimmed['last_name'])) {
		...
		$ln=trim(htmlspecialchars($_POST['last_name']));
	} else {
	...
	if (filter_var($trimmed['email'], FILTER_VALIDATE_EMAIL)) {
		...
		$e=trim(htmlspecialchars($_POST['email']));
	} else {
...
$row=$r->fetch();
		if ($row==null) { // Available.
			...
			$a="";
			$q = "INSERT INTO users (first_name, last_name,email, password, active, registration_date) VALUES ( '$fn', '$ln','$e', '$p', '$a', NOW() )";
			...
			//if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
				...
				//mail($trimmed['email'], 'Registration Confirmation', $body, 'From: leilanajafi76@gmail.com');
				mail($e, 'Registration Confirmation', $body, 'From: leilanajafi76@gmail.com');
